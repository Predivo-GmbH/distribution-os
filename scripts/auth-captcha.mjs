#!/usr/bin/env node
/**
 * auth-captcha.mjs — read and flip Distribution-OS's Supabase Auth CAPTCHA switch, without ever
 * rendering a secret.
 *
 ...
 * (app/src/hooks/useAuth.ts, rendered by Login/SignUp/ResetPassword) — ships with the deploy.
 ...
 * how Distribution-OS's gets flipped, read back and rolled back, on the record, by CI, with no
 * secret rendered anywhere.
 *
 * WHAT IT DOES
 ...
 * token on /token (password login), /signup, /otp (login code + signup code), /resend and /recover
 * (password reset). Distribution-OS calls every one of those from the browser, so enabling before
 * the client half is live on that project breaks login, signup AND password reset at once. Prove
 * the client half first — both deploy workflows now assert the site key is actually inlined in the
 * shipped bundle.
 *
 * STAGING IS DELIBERATELY NOT ENABLED. There is exactly one captcha secret per Supabase project,
 * so there is no "test key" that could be scoped to staging alone, and staging's own e2e suite
 * signs in for real. The rationale is carried in
...
const PROJECTS = {
  production: { ref: 'jxjpbmkgmuunpayqgbsx', name: 'Distribution-OS production' },
  staging: { ref: 'jckctrtkstejolddqzlk', name: 'Distribution-OS staging' },
}

/** PUBLIC Cloudflare Turnstile site key for Distribution-OS — safe to record, it ships in every bundle. */
const SITE_KEY = '0x4AAAAAAEz4thUGbxYbUP3e'
...
const MGMT_CANDIDATES = [
  { name: 'DIST_MGMT_TOKEN', token: process.env.DIST_MGMT_TOKEN || '' },
...
async function mgmt(path, init = {}) {
  if (!MGMT_CANDIDATES.length) {
    throw new Error('No Supabase management token in the environment (DIST_MGMT_TOKEN or SUPABASE_ACCESS_TOKEN).')
  }
...
function turnstileSecret() {
  ...
  if (!value) {
    throw new Error(
      'TURNSTILE_SECRET is not set. It is the Cloudflare Turnstile SECRET key for the ' +
        `Distribution-OS sign-in widget (site key ${SITE_KEY}). Add it as the repository secret ` +
  ...
  if (codes.includes('invalid-input-secret') || codes.includes('missing-input-secret')) {
    throw new Error(
      `Cloudflare REFUSED this secret (${codes.join(', ')}). TURNSTILE_SECRET is not the secret key ` +
        `for the Distribution-OS widget (site key ${SITE_KEY}). Enabling captcha with it would reject ` +
...
async function main() {
    ...
    if (key === 'staging') {
      console.error(
        'REFUSED - staging is deliberately left open. One captcha secret per Supabase project, so ' +
          ...
          "QA and this repo's own staging e2e sign-in. The rationale lives in " +
  ...
  if (want) {
    ...
    console.log('Then DELETE the signin-captcha.prod.test.mjs entry from scripts/guards-known-failing.json —')
    console.log('run-guards.mjs fails while a listed suite passes, so the baseline clears itself.')
